/**
 * Smoke test — Hono e-commerce fixture.
 * Run with:  npx ts-node tests/smoke-hono.ts   (from repo root)
 *
 * Verifies:
 * 1. AST parser handles a Hono codebase with chained-style routes (app.get/post/put/delete).
...
const SIDECAR_PORT = 4575;
const FIXTURE_DIR = path.resolve(__dirname, 'hono-ecommerce/src');
...
async function run(): Promise<void> {
  console.log('--- Hono e-commerce smoke test ---\n');

  // 1. Parse the full Hono fixture (chained routes + service classes)
  ...
  if (graph.nodes.length < 8) {
    throw new Error(`Expected ≥8 nodes from a multi-module Hono app, got ${graph.nodes.length}`);
  }
  ...
  // 2. HTTP methods on route nodes (app.get / app.post etc.)
  ...
  // 4. Cache round-trip
  const cache = new CacheService('/tmp/flow-map-smoke-hono');
  const hash = cache.hashBody("app.get('/orders', handler)");
  cache.set('hono-smoke::listOrders', hash, 'List all orders');
  const hit = cache.get('hono-smoke::listOrders', hash);
  ...
  await sidecar.stop();
  console.log('\n✓ Hono smoke test passed');
}

run().catch((err) => {
  console.error('\n✗ Hono smoke test failed:', err.message);
  process.exit(1);
});
